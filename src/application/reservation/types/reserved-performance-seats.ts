...
export type PerformanceStagingWithSeats = PerformanceStagingDateInfo & {
  seats: PerformanceSeatInfo[];
};
export type ReservedPerformanceSeats = Pick<
  Performance,
  'id' | 'title' | 'ticketingStartDate'
> & {
  stage: Omit<Stage, 'dateCreated'>;
  performanceStagingDate: PerformanceStagingWithSeats[];
};
